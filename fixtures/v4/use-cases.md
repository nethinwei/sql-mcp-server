...
> **非验收交付物**。频率排序未经生产数据或 dogfooding 验证，仅供
> `v0.1.10` 诊断任务选材与社区讨论参考。正式 Eval 任务见
> `tasks-v5/`（[Diagnostic Evaluation 设计文档](../../docs/design/diagnostic-evaluation.md)）。

`v0.1.9` 已交付 21 个 Guided Eval 任务；本目录**不**作为版本退出门禁。

按电商、支付、账务、直播常见运营场景的**主观频率**从高到低排列（排名 1
≈ 更常见日报查询）。每条标注语义陷阱与现有 Eval 覆盖关系。
...
`v0.1.9` 的 21 个 Eval 任务覆盖各模块高频区中的语义陷阱子集。`v0.1.10`
从本草案选材扩展诊断任务集（`tasks-v5/`），**不要求** 128 条全部纳入 Eval。
