...
[testing.md](testing.md)。[威胁模型](threat-model.md)把攻击场景映射到这些不变量
及其他控制与证据；映射关系不把单元测试提升为形式化证明。
...
provider 路由时，应明确指出受影响的不变量与 threat ID，并增加对应回归测试。
若代码事实与本文件不一致，应先修正事实或降低本文件中的保证，不能只更新宣传
描述；threat 证据状态也应随实际测试范围更新。
