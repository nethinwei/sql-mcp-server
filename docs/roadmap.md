...
## Committed — v0.1.4 Security Assurance Baseline

### 目标

把现有安全说明升级为可审阅、可回归、可持续验证的安全证明基线，降低后续控制面
扩大配置、身份和 token 攻击面时的回归风险。

### 范围

- 新增 threat model，覆盖资产、信任边界、攻击者、安全假设、攻击入口、防御措施、
  剩余风险和非保证范围；
- 建立公开 adversarial corpus，优先覆盖 identifier/schema confusion、字段侧信道、
  row policy 绕过、身份与 transaction token 混淆、trusted proxy spoofing、
  跨租户缓存污染、reload 竞态和 oversized payload；
- 为 MCP payload、IR validator、codegen 和 transaction state machine 建立首批
  fuzz target 与属性测试；
- 持续验证值与标识符隔离、输出字段收敛、row policy 不被用户 filter 弱化，以及
  失败路径不遗留开放事务；
- 校准 README 与安全文档中的宣称，使其不超过测试能够证明的范围。
...
- 核心安全属性在 PostgreSQL、MySQL 和 OceanBase 的适用范围有明确证据或限制说明。

### 非目标

- 新 Provider 或兼容性认证；
- 控制面、管理 UI 或新的 MCP 工具；
- 完整数据库权限 doctor、外部安全审计或通用 ABAC；
- durable / fail-closed audit。

---

## Next — v0.1.5 Data Plane Contract Hardening

### 进入条件

`v0.1.4` 验收完成，且现有客户端兼容策略已明确。
